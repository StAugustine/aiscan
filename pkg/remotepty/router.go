...
func (r *Router) attachExisting(ctx context.Context, frame Frame, info tmux.Info, send func(Frame)) {
	n := frame.Bytes
	if n <= 0 {
		n = r.attachBytes
	}
...
func (r *Router) input(frame Frame, send func(Frame)) {
	...
	if info, ok := r.mgr.Get(sessionID); ok && info.State != tmux.StateRunning {
		return
	}
...
func (r *Router) findReusableSession(kind, name string) (tmux.Info, bool) {
	if r.mgr == nil {
		return tmux.Info{}, false
	}
	var fallback tmux.Info
	hasFallback := false
	for _, info := range r.mgr.List() {
		if info.State != tmux.StateRunning || strings.ToLower(strings.TrimSpace(info.Kind)) != kind {
			continue
		}
