...
func (m *fakeManager) add(id, name string, output []byte) tmux.Info {
	return m.addKind(id, "", name, output)
}

func (m *fakeManager) addKind(id, kind, name string, output []byte) tmux.Info {
	info := tmux.Info{ID: id, Name: name, State: tmux.StateRunning}
	if kind != "" {
		info.Kind = kind
	}
...
func (m *fakeManager) List() []tmux.Info {
	out := make([]tmux.Info, 0, len(m.sessions))
...
func (m *fakeManager) Get(id string) (tmux.Info, bool) {
	s, ok := m.sessions[id]
	if !ok {
		return tmux.Info{}, false
	}
	return s.info, true
}
...
func (m *fakeManager) Wait(ctx context.Context, id string, timeout time.Duration) (tmux.Info, error) {
	s := m.sessions[id]
	if s == nil {
		return tmux.Info{}, errors.New("missing session")
	}
	select {
	case <-ctx.Done():
